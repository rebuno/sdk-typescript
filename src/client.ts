...
export class Client {
  ...
  constructor(opts: ClientOptions = {}) {
    ...
    if (!url)
      throw new Error(
        "Client baseUrl is required (set REBUNO_URL or pass baseUrl)",
      );
  ...
  async events(
    ...
    return (((await r.json()) as unknown[]) ?? []).map((e) =>
  ...
  async listSteps(
    ...
    const r = await this.request("GET", `/v0/executions/${executionId}/steps`, {
      query,
    });
    return (((await r.json()) as unknown[]) ?? []).map((s) =>
      parseStep(s as Record<string, unknown>),
    );
  }
  ...
  async listApprovals(opts: { status?: string } = {}): Promise<Approval[]> {
    const r = await this.request("GET", "/v0/approvals", {
      query: { status: opts.status ?? "pending" },
    });
    return (((await r.json()) as unknown[]) ?? []).map((a) =>
      parseApproval(a as Record<string, unknown>),
    );
  }
  ...
  async grantApproval(
    approvalId: string,
    opts: { decidedBy: string; rationale?: string },
  ): Promise<void> {
    await this.request("POST", `/v0/approvals/${approvalId}/grant`, {
      body: { decided_by: opts.decidedBy, rationale: opts.rationale ?? "" },
    });
  }
  ...
  async denyApproval(
    ...
    await this.request("POST", `/v0/approvals/${approvalId}/deny`, {
      body: { decided_by: opts.decidedBy, rationale: opts.rationale ?? "" },
    });
  }
}
